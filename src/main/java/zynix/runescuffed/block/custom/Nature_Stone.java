...
public class Nature_Stone extends Block {
    public Nature_Stone(Settings settings) {
        super(settings);
    }
    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        if (!world.isClient() && entity instanceof ItemEntity itemEntity) {
            ...
            if (isValidItem(inputStack)) {
                ...
                itemEntity.setStack(new ItemStack(ModItems.NATURE_RUNE, newCount));

                world.playSound(null, pos, SoundEvents.BLOCK_AZALEA_LEAVES_FALL, SoundCategory.BLOCKS, 1f, 1f);

                if (world instanceof net.minecraft.server.world.ServerWorld serverWorld) {
                    serverWorld.spawnParticles(ParticleTypes.PALE_OAK_LEAVES,
